        ...
        if self.enable_flash and q.device.type == "cuda":
            ...
            if FLASH_AVAILABLE:
                if adj_mat is not None:
                    v_add = torch.einsum(
                        "...ij,...jd->...id",
                        F.softmax(adj_mat, -1).unsqueeze(1),
                        v,
                    )
                args = [x.half().contiguous() for x in [q, k, v]]
                v = F.scaled_dot_product_attention(*args, attn_mask=mask).to(
                    q.dtype
                )
                if adj_mat is not None:
                    v += v_add
...
class CrossBlock(nn.Module):
    ...
    def forward(
        ...
        x0: torch.Tensor,
        x1: torch.Tensor,
        encoding0: torch.Tensor = None,
        encoding1: torch.Tensor = None,
        mask: Optional[torch.Tensor] = None,
    ) -> List[torch.Tensor]:
        ...
        if self.flash is not None and qk0.device.type == "cuda":
            ...
            m1 = torch.einsum(
                "bhji, bhjd -> bhid", attn10.transpose(-2, -1), v0
            )
            if mask is not None:
...
class AdjBlock(nn.Module):
    def __init__(
        ...
        embed_dim: int,
        num_heads: int = 1,
        bias: bool = True,  # , slope: float = 0.1
    ) -> None:
        super().__init__()
        self.heads = num_heads
        # self.slope = slope
    ...
    def forward(
        ...
        if mask0 is not None and mask1 is not None:
            ...
            for i in range(self.atten_layers):
                ...
                desc1 = self.self_attn[i](desc1, encoding1)
                cross_embd = self.embd_layers[i](embd).type(desc0.dtype)
                feature0_embd = self.feature_embd_layers[i](feature0_embd).type(
                    desc0.dtype
                )
                feature1_embd = self.feature_embd_layers[i](feature1_embd).type(
                    desc0.dtype
                )
                desc0_adj = desc0 + cross_embd.unsqueeze(1) + feature0_embd.unsqueeze(1)
                desc1_adj = desc1 + cross_embd.unsqueeze(1) + feature1_embd.unsqueeze(1)
                desc0_adj, desc1_adj = self.adj_attn[i](
                    desc0_adj, desc1_adj, adj_mat
                )
                desc0, desc1 = self.desc_compact[i](
...
def sigmoid_log_double_softmax(
    ...
    scores1 = F.log_softmax(sim.transpose(-1, -2).contiguous(), 2).transpose(
        -1, -2
    )
...
class DiffGlue(nn.Module):
    ...
    def __init__(self, conf) -> None:
        ...
        self.transformers = nn.ModuleList(
            [
                TransformerLayer(d, h, self.time_embed_channels, conf.flash)
